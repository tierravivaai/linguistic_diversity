#!/usr/bin/env python3
"""
Create a global heat map of language endangerment by country.

Uses a cream-to-dark-red colour scale keyed to each country's own endangerment %,
with labels for each UN sub-region showing name, percentage, and raw endangered count.
"""
...
def main():
    ...
    # --- Aggregate by individual country (deduplicated) ---
    country_langs = ex.drop_duplicates(subset=["glottocode", "iso_a2"])
    country_stats = (
        country_langs.groupby("iso_a2")
        .agg(
    ...
    country_stats["pct_endangered"] = (
        country_stats["endangered"] / country_stats["total_languages"] * 100
    ).round(1)

    # Map country-level pct to each country polygon
    country_pct_map = country_stats.set_index("iso_a2")["pct_endangered"].to_dict()
    world["country_pct"] = world["ISO_A2_EH"].map(country_pct_map)

    # --- Also compute sub-region stats for labels ---
    ...
    # Plot each country coloured by its own endangerment %
    world_proj.dropna(subset=["country_pct"]).plot(
        ax=ax,
        column="country_pct",
        cmap=cmap,
        norm=norm,
    ...
    no_data = world_proj[world_proj["country_pct"].isna()]
    if len(no_data) > 0:
        no_data.plot(
            ax=ax,
            color="#C7CDD4",
    ...
    fig.text(
        0.43,
        0.91,
        "Language Endangerment by Country",
        ha="center",
        va="top",
        fontsize=20,
        fontweight="bold",
